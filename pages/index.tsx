        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6 p-2 md:p-6">
          {posts.map((post) => (
            <Link key={post._id} href={`/post/${post.slug.current}`}>
              <div className="flex flex-col border rounded-lg group cursor-pointer overflow-hidden">
                {post.mainImage && (
                  <img
                ...
                {/* Author */}
                <div className="flex mt-auto justify-between p-5 bg-white w-full h-15">
                      <p className="pt-3">{ post.author && <span className="font-bold">{post.author.name}</span>}</p> 
                       { post.author &&  <img
